...
def train(args):
    with open(args.config_path, 'r') as file:
        try:
            ...
            print(f"Error parsing YAML configuration file: {exc}")
        print(config)
    ...
    for epoch_idx in range(num_epochs):
        ...
        for im in tqdm(data_loader):
            ...
            if step_count % image_save_steps or step_count == 1:
                ...
                grid = make_grid(
                    torch.cat([save_input, save_output], dim=0),
                    nrow=sample_size)
                ...
                img.save(os.path.join(train_config['task_name'], 'vqvae_autoencoder_samples', f'current_autoencoder_sample_{img_save_count}.png'))
                img_save_count += 1
                img.close()
            ...
            if step_count > disc_step_start:
                fake = output
                disc_fake_pred = descriminator(fake.detach())
                disc_real_pred = descriminator(im)
